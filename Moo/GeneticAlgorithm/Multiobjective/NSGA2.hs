...
import Data.Array.ST (STArray, getBounds, getElems, newArray, readArray, runSTArray, writeArray)
...
-- are not worse.
domination ::
  -- | problem types per every objective
  [ProblemType] ->
  DominationCmp a
...
data RankedSolution a
  = RankedSolution
      { rs'phenotype :: MultiPhenotype a,
        -- | @0@ is the best
        rs'nondominationRank :: Int,
        -- | @Infinity@ for less-crowded boundary points
...
nsga2Ranking ::
  ...
  MultiObjectiveProblem fn ->
  -- | @n@, number of top-ranked genomes to select
  Int ->
  -- | a population of raw @genomes@
  [Genome a] ->
  -- | selected genomes with their non-domination ranks
...
stepNSGA2 problems select crossover mutate stop input = do
  ...
  case input of
    (Left _) ->
      -- raw genomes => it's the first generation
      stepNSGA2'firstGeneration dominates problems select crossover mutate stop input
    (Right _) ->
      -- ranked genomes => it's the second or later generation
...
stepNSGA2'poolSelection ::
  ...
  -- | a list of @objective@ functions
  MultiObjectiveProblem fn ->
  -- | @n@, the number of solutions to select
  Int ->
  -- | @pool@ of genomes to select from
  [Genome a] ->
  -- | @n@ best phenotypes
  [Phenotype a]
stepNSGA2'poolSelection dominates problems n pool =
  ...
  let rankedgenomes =
        let grs = nsga2Ranking dominates problems n pool
