    ...
    [ "tournamentSelect" ~: do
      ...
      "tournamentSelect (10 times best of 4, seed 1)" ~: do
        ...
        assertEqual "maximum is selected every time" (replicate times True) $
          zipWith
            (\selected xs -> selected == (maximum . map takeObjectiveValue $ xs))
            objVals
            samples,
      "rouletteSelect" ~: do
        ...
        let numOfNines =
              length . filter (== 9.0) . map takeObjectiveValue
                . flip evalRand (pureMT 1)
                $ rouletteSelect tries
                $ gs
      ...
      "stochasticUniversalSampling" ~: do
        ...
        assertEqual "counts are fitness proportional" [4, 8] $
          map
            length
            [ (filter ((== 1) . takeObjectiveValue) selected),
